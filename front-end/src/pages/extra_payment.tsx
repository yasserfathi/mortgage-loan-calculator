...
function LoanDashboard() {
  const [extra_payment, setExtra_payment] = useState<Extra_payment[]>([]);
  ...
  const validateForm = (): boolean => {
    ...
    // Extra Payment
    if (!formData.extra_payment) {
      newErrors.extra_payment = 'Extra Payment is required';
    } else if (isNaN(termYears)) {
      newErrors.extra_payment = 'Must be a valid number';
    } else if (termYears <= 0) {
      newErrors.extra_payment = 'Must be greater than 0';
    }
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/loans', {
        ...
        body: JSON.stringify({
          ...
          extra_payment: parseInt(formData.extra_payment)
        }),
      });
      ...
      // Get Extra_payment Data
      const updatedResponse = await fetch('http://127.0.0.1:8000/api/extra_payment/'+resultData.data.loan_id);
      const response_data = await updatedResponse.json();
      setExtra_payment(response_data.data)
      
      setFormData({ principal: '', annual_interest_rate: '', term_years: '', extra_payment: '' });
      setTable(true);
      setShowModal(false);
    } catch (err) {
      ...
      <Card className="shadow-sm">
        <Card.Body>
          <Table striped bordered hover>
            ...
            <tbody>
              {extra_payment.length > 0 ? (
                extra_payment.map(extra_payment => (
                  <tr key={extra_payment.month_number} style={{ cursor: 'pointer' }}>
                    <td>{extra_payment.month_number}</td>
                    <td>{extra_payment.starting_balance}</td>
                    <td>{extra_payment.monthly_payment}</td>
                    <td>{extra_payment.principal_component}</td>
                    <td>{extra_payment.interest_component}</td>
                    <td>{extra_payment.ending_balance}</td>
                    <td>{extra_payment.remaining_loan_term}</td>
                  </tr>
                ))
              ) : (
                <tr>
          ...
          <Form onSubmit={handleSubmit} noValidate>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control 
                ...
                min="1"
                max="50"
                isInvalid={!!errors.extra_payment}
              />
              <Form.Control.Feedback type="invalid">
